...
public class GunController : MonoBehaviour
{
    ...
    public string crosshairObjectName = "Aim"; // UI 씬에서의 오브젝트 이름
    public Color normalColor = Color.white;
    public Color targetDetectedColor = Color.green;
    ...
    void Update()
    {
        ...
        // 1. UI가 아직 할당되지 않았다면 동적으로 찾기
        if (crosshairImage == null) FindCrosshair();

        // 2. 우클릭(조준) 상태일 때만 레이캐스트 및 색상 변경 실행
        if (Input.GetMouseButton(1))
        {
    ...
    void FindCrosshair()
    {
        // UI 씬이 Additive로 로드되었으므로 이름으로 찾습니다.
        GameObject go = GameObject.Find(crosshairObjectName);
        if (go != null)
        {
    ...
    void UpdateCrosshairStatus()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 1000f))
        {
            ...
            {
                crosshairImage.color = Color.yellow; // Wall → 노란색
            }
            else if (hit.collider.CompareTag("Ceiling"))
            {
                crosshairImage.color = Color.red;    // Ceiling → 빨간색
            }
            else
            {
                crosshairImage.color = targetDetectedColor; // 그 외 → 기본 감지 색
            }
        }
        else
        {
            crosshairImage.color = normalColor; // 아무것도 맞지 않음
        }
    }
